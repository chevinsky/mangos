...
void Spell::finish(bool ok)
{
    ...
    if ((m_caster->GetTypeId() == TYPEID_PLAYER || m_caster->GetVehicleKit()) && NeedsComboPoints(m_spellInfo))
    {
        ...
        if(_target)                                         //for target dead/target not valid
        {
            ...
            else
            {
                ...
                if(m_caster->IsFriendlyTo(target) && !duelvsplayertar && !IsDispelSpell(m_spellInfo) &&
                    !m_caster->GetVehicleKit() && m_caster->GetCharmerOrOwner() && m_caster->GetCharmerOrOwner()->IsFriendlyTo(target))
                {
                    return SPELL_FAILED_BAD_TARGETS;
                }
            }
