...
void ArenaTeamMember::ModifyMatchmakerRating(Player* plr, int32 mod, int type)
{
    if(type == ARENA_TYPE_2v2) {
        if (int32(matchmaker_rating) + mod <  0)
            ...
            CharacterDatabase.PExecute("UPDATE arena_mmr_rating SET rating2 = '%u' WHERE guid = '%u'",matchmaker_rating, plr->GetObjectGuid().GetCounter());
        }
    }
    if(type == ARENA_TYPE_3v3) {
        if (int32(matchmaker_rating) + mod <  0)
            ...
            matchmaker_rating += mod;
            CharacterDatabase.PExecute("UPDATE arena_mmr_rating SET rating3 = '%u' WHERE guid = '%u'",matchmaker_rating, plr->GetObjectGuid().GetCounter());
        }
    }
    if(type == ARENA_TYPE_5v5) {
        if (int32(matchmaker_rating) + mod <  0)
            ...
            matchmaker_rating += mod;
            CharacterDatabase.PExecute("UPDATE arena_mmr_rating SET rating5 = '%u' WHERE guid = '%u'",matchmaker_rating, plr->GetObjectGuid().GetCounter());
        }
    }
}
...
bool ArenaTeam::AddMember(ObjectGuid playerGuid)
{
    ...
    if(GetType() == ARENA_TYPE_2v2) {
        QueryResult *result = CharacterDatabase.PQuery("SELECT rating2 FROM arena_mmr_rating WHERE guid='%u'", playerGuid.GetCounter());

        if(!result) {
            CharacterDatabase.PExecute("INSERT INTO arena_mmr_rating (guid, rating2, rating3, rating5) VALUES""('%u', '%u', '%u', '%u')",playerGuid.GetCounter(),1500,1500,1500);
    ...
    if(GetType() == ARENA_TYPE_3v3) {
        QueryResult *result = CharacterDatabase.PQuery("SELECT rating3 FROM arena_mmr_rating WHERE guid='%u'", playerGuid.GetCounter());
    ...
    if(GetType() == ARENA_TYPE_5v5) {
        QueryResult *result = CharacterDatabase.PQuery("SELECT rating5 FROM arena_mmr_rating WHERE guid='%u'", playerGuid.GetCounter());
...
bool ArenaTeam::LoadMembersFromDB(QueryResult *arenaTeamMembersResult)
{
    ...
    do
    {
        ...
        if(GetType() == ARENA_TYPE_2v2) {
            QueryResult *result = CharacterDatabase.PQuery("SELECT rating2 FROM arena_mmr_rating WHERE guid='%u'", fields[1].GetUInt32());

            if(!result) {
                CharacterDatabase.PExecute("INSERT INTO arena_mmr_rating (guid, rating2, rating3, rating5) VALUES""('%u', '%u', '%u', '%u')",fields[1].GetUInt32(),1500,1500,1500);
                newmember.matchmaker_rating = 1500;
            }  
            else {
                newmember.matchmaker_rating = (*result)[0].GetUInt32();
        ...
        if(GetType() == ARENA_TYPE_3v3) {
            QueryResult *result = CharacterDatabase.PQuery("SELECT rating3 FROM arena_mmr_rating WHERE guid='%u'", fields[1].GetUInt32());
        ...
        if(GetType() == ARENA_TYPE_5v5) {
            QueryResult *result = CharacterDatabase.PQuery("SELECT rating5 FROM arena_mmr_rating WHERE guid='%u'", fields[1].GetUInt32());
        ...
        {
            CharacterDatabase.PExecute("DELETE FROM arena_mmr_rating where guid = '%u'",fields[1].GetUInt32());
...
void ArenaTeam::OfflineMemberLost(ObjectGuid guid, uint32 againstRating)
{
    ...
    for(MemberList::iterator itr = m_members.begin(); itr !=  m_members.end(); ++itr)
    {
        if (itr->guid == guid)
        {
            ...
            if(GetType() == ARENA_TYPE_2v2)
                CharacterDatabase.PExecute("UPDATE arena_mmr_rating SET rating2 = '%u' WHERE guid = '%u'",itr->matchmaker_rating, guid.GetCounter());
            if(GetType() == ARENA_TYPE_3v3) 
                CharacterDatabase.PExecute("UPDATE arena_mmr_rating SET rating3 = '%u' WHERE guid = '%u'",itr->matchmaker_rating, guid.GetCounter());
            if(GetType() == ARENA_TYPE_5v5) 
                CharacterDatabase.PExecute("UPDATE arena_mmr_rating SET rating5 = '%u' WHERE guid = '%u'",itr->matchmaker_rating, guid.GetCounter());
